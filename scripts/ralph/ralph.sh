...
# Extract Z AI credentials from providers.yaml
PROVIDERS_FILE="$HOME/.sh/providers.yaml"
if [[ -f "$PROVIDERS_FILE" ]]; then
  # Extract Z AI credentials
  ANTHROPIC_AUTH_TOKEN=$(awk '
    /- name:/ {
      if ($0 ~ "\"Z AI\"") found=1
      else found=0
    }
